...
# fd = gen_obscured_df(fd, logNH_c)

# df_columns = fd["df_new"].columns
fig = px.line(
    fd["df"],
    x=fd["df"]["E_keV"],
    y=fd["df"]["lognh_%.2f" %(logNH_c)],
    log_x=True,
    log_y=True,
    width=1000,
    height=500,
...
colour = "rgba(%s)" %(",".join(["%.5f" %(f) for f in fd["cmap_cols"][fd["df"].columns.get_loc("lognh_%.2f" %(logNH_c)) - 1]]))
...
if st.checkbox("Show Table", False):
    ...
    st.write(fd["df"][["E_keV", "lognh_%.2f" %(logNH_c)]], index=False)
